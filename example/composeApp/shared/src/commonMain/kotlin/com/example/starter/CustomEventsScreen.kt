...
internal fun CustomEventsScreen(onBack: () -> Unit) =
    DemoScaffold("Custom Events", "Custom Events", onBack) { report ->
        ...
        DemoButton("Event without properties") {
            UXCam.logEvent("event_no_props"); report("logEvent(\"event_no_props\")")
        }
        ...
        DemoButton("Event with empty map") {
            UXCam.logEvent("event_empty_map", emptyMap()); report("logEvent(\"event_empty_map\", {})")
        }
        DemoButton("Event with null map") {
            UXCam.logEvent("event_null_map", null); report("logEvent(\"event_null_map\", null)")
        }
        ...
        DemoButton("Event with empty JSON") {
            UXCam.logEventWithJson("event_empty_json", "{}"); report("logEventWithJson(\"event_empty_json\", \"{}\")")
        }
        DemoButton("Event with null JSON") {
            UXCam.logEventWithJson("event_null_json", null); report("logEventWithJson(\"event_null_json\", null)")
        }
        ...
        DemoButton("Log custom event") {
            UXCam.logEvent(eventName, mapOf(key to value))
