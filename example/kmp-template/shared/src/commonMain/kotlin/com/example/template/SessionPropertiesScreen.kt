...
internal fun SessionPropertiesScreen(onBack: () -> Unit) =
    DemoScaffold("Session Properties", "Session Properties", onBack) { report ->
        ...
        DemoButton("Set string property (tier = gold)") {
            UXCamKMP.setSessionProperty("tier", "gold"); report("setSessionProperty(\"tier\", \"gold\")")
        }
        DemoButton("Set int property (cart_items = 3)") {
            UXCamKMP.setSessionProperty("cart_items", 3); report("setSessionProperty(\"cart_items\", 3)")
        }
        DemoButton("Set float property (cart_total = 49.99)") {
            UXCamKMP.setSessionProperty("cart_total", 49.99f); report("setSessionProperty(\"cart_total\", 49.99)")
        }
        ...
        DemoButton("Mark session as favorite") {
            UXCamKMP.markSessionAsFavorite(); report("markSessionAsFavorite()")
        }
        ...
        DemoButton("Set custom property") {
            UXCamKMP.setSessionProperty(key, value); report("setSessionProperty(\"$key\", \"$value\")")
        }
    }
